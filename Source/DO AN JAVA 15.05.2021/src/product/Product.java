...
//                            + " VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public Object[][] getListWithOptions() throws SQLException {
        ArrayList<Product> products = new ArrayList<>();
        Connection conn = ConnectDB.connectDB();
        ...
        Object[][] objArray = new Object[products.size()][];
        for (int i = 0; i < products.size(); i++) {
            objArray[i] = new Object[]{products.get(i).getProduct_id(),products.get(i).getProduct_name(),  products.get(i).getProductPrice(), products.get(i).getSoLuong()};
        }
        return objArray;
    }
