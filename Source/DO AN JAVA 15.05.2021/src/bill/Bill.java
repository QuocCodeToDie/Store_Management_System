...
public class Bill {
    ...
    public bill_detail  inCTBill(int id_bill) throws SQLException {
        Connection conn = ConnectDB.connectDB();
        ArrayList<bill_detail> bill_details = new ArrayList<>();
        String qSelect = "{ call quanlycuahang.xemChiTietBill(?) }";
        PreparedStatement preparedStmt = conn.prepareStatement(qSelect);
        ...
        bill_detail bd1 = new bill_detail();
        ArrayList<bill_detail> tmp = new ArrayList<>();
        while(rs.next()){
            int id_sp = rs.getInt("idSanPham");
            String sp_name = rs.getString("tenSanPham");
            ...
            int don_gia = rs.getInt("gia");

            bd1 = new bill_detail(id_bill,id_sp,sp_name,soluong,date,tongtien,id_nv,id_kh,don_gia);
            //tmp.add(bd1);
        }
        conn.close();
        return bd1;
