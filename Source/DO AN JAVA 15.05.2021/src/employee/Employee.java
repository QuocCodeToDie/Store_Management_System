...
public class Employee {
    ...
    public String geteNameByID(int id) throws SQLException {
        Connection conn = ConnectDB.connectDB();
        String query1 ="select  idNhanVien from quanlycuahang.nhanvien where idNhanVien = "+id;
        PreparedStatement preparedStmt1 = conn.prepareStatement(query1);
        ResultSet temp_list_emp = preparedStmt1.executeQuery();
        String name = "";
        while(temp_list_emp.next()){
            name= String.valueOf(temp_list_emp.getInt("idNhanVien"));
        }
        conn.close();
        return name;
